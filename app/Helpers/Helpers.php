...
if (! function_exists('send_notification_email_to_users')) {
...
if (! function_exists('trial_checker')) {
	...
	{
		$membership_logs = DB::table('membership_logs')->where('user_id', Auth::user()->id)->count();
		if ($membership_logs > 0) {
			$membership_logs = DB::table('membership_logs')->where('user_id', Auth::user()->id)->where('membership_end', '>=', date('Y-m-d'))->count();
			if ($membership_logs > 0) {
				return [
					'status' => 0,
			...
			$trialExpiresOn = Carbon::parse(Auth::user()->trial_expires_on);
			...
			if ($trialExpiresOn->isFuture()) {
				...
				$hours = $remainingTime->h + ($days * 24);
				return [
					'status' => 2,
					'message' => sprintf(
						'You are using a free trial. Your trial will expire in %d day%s and %d hour%s.',
						$days,
						$days !== 1 ? 's' : '',
						$hours % 24,
						($hours % 24) !== 1 ? 's' : ''
